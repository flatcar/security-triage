...
def test_runner_has_no_direct_apply_discovery_method():
    """discovery must never write issues directly; apply_discovery must not exist."""
    runner = GitHubActionRunner(FakeIssueClient(), ActionFlags())
    assert not hasattr(runner, "apply_discovery"), (
        "GitHubActionRunner must not expose apply_discovery; "
        "issue creation is gated through review apply only"
    )


def test_runner_has_no_direct_apply_cleanup_method():
    """cleanup must never write issues directly; apply_cleanup must not exist."""
    runner = GitHubActionRunner(FakeIssueClient(), ActionFlags())
    assert not hasattr(runner, "apply_cleanup"), (
        "GitHubActionRunner must not expose apply_cleanup; "
        "cleanup mutations are gated through review apply only"
    )
