...
def load_structured_file(path: str | Path) -> Any:
    ...
    try:
        ...
        try:
            ...
            raise DataFormatError(f"Could not parse {file_path} as JSON and PyYAML is not installed") from exc
        return yaml.safe_load(text)
...
def write_yaml_file(path: str | Path, data: Any) -> None:
    try:
        ...
        raise DataFormatError("YAML output requires PyYAML; choose --format json or install the yaml extra") from exc
    Path(path).parent.mkdir(parents=True, exist_ok=True)
