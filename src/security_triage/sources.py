...
OPTIONAL_SOURCE_SPECS = [
    SourceSpec("redhat", REDHAT_VULNERABILITIES_URL, optional=True)
]
...
def load_source_fixture(
    ...
    if not isinstance(items, list):
        raise ValueError(
            "Source fixture must be a list or an object with an 'entries' list"
        )
...
def source_entry_from_mapping(item: dict[str, Any]) -> SourceEntry:
    ...
    content = str(
        item.get("content") or item.get("raw_source_excerpt") or item.get("body") or ""
    )
    source_url = str(item.get("source_url") or item.get("url") or "")
    entry_id = str(
        item.get("entry_id")
        or item.get("id")
        or item.get("raw_advisory_id")
        or source_url
        or title
    )
    ...
    return SourceEntry(
        ...
        new_comments=_dict_list(
            item.get("new_comments")
            or item.get("upstream_new_comments")
            or item.get("upstream_recent_comments")
        ),
...
def fetch_live_sources(
    ...
    selected_specs = specs or [
        *DEFAULT_SOURCE_SPECS,
        *(OPTIONAL_SOURCE_SPECS if include_optional else []),
    ]
    ...
    for spec in selected_specs:
        try:
            ...
            if spec.name == "gentoo":
                ...
                fetched = fetch_oss_security_entries(
                    start,
                    end,
                    cache_dir=oss_security_cache_dir,
                ...
                fetched = fetch_go_vulndb_entries(
                    start, end, cache_dir=go_vulndb_cache_dir, progress_logger=progress
                )
                filtered = filter_entries_by_window(
                    fetched,
                    start,
                ...
                fetched = fetch_rustsec_entries(
                    start, end, cache_dir=rustsec_cache_dir, progress_logger=progress
                )
                filtered = filter_entries_by_window(
                    fetched,
                    start,
            ...
            progress.info(
                f"Source {spec.name}: fetched {len(fetched)} entr{'y' if len(fetched) == 1 else 'ies'}, kept {len(filtered)} in window"
            )
...
def fetch_gentoo_entries(
    start: str | None = None, end: str | None = None
) -> list[SourceEntry]:
    try:
        payload = fetch_json(
            _gentoo_bugzilla_search_url(start, end), accept="application/json"
        )
        bugs = payload.get("bugs", []) if isinstance(payload, dict) else []
        comments_by_bug = fetch_gentoo_comments(
            [str(bug.get("id")) for bug in bugs if bug.get("id")]
        )
        entries: list[SourceEntry] = []
        for bug in bugs:
            ...
            description, comments = _split_bugzilla_comments(
                comments_payload, bug.get("creator_detail")
            )
            ...
            entries.append(
                SourceEntry(
                    ...
                    entry_id=bug_id,
                    title=summary,
                    content=_gentoo_content(
                        bug_id, summary, url, metadata, description, new_comments
                    ),
...
def fetch_gentoo_comments(
    bug_ids: list[str], batch_size: int = 100
    ...
    for batch in _batches(ids, batch_size):
        ...
        try:
            comments.update(
                _comments_from_bugzilla_payload(
                    fetch_json(
                        _gentoo_bugzilla_comments_url(batch), accept="application/json"
                    )
                )
            )
        except Exception:
            for bug_id in batch:
                try:
                    comments.update(
                        _comments_from_bugzilla_payload(
                            fetch_json(
                                f"{GENTOO_BUGZILLA_BASE_URL}/rest/bug/{bug_id}/comment",
...
def _split_bugzilla_comments(
    raw_comments: list[dict[str, Any]], creator_detail: Any
...
def _normalize_bugzilla_comment(
    raw_comment: dict[str, Any], creator_identities: set[str]
) -> dict[str, Any]:
    raw_creator_detail = raw_comment.get("creator_detail")
    creator_detail: dict[str, Any] = (
        raw_creator_detail if isinstance(raw_creator_detail, dict) else {}
    )
    creator_values = _creator_identities(creator_detail)
    creator = str(
        raw_comment.get("creator")
        or creator_detail.get("email")
        or creator_detail.get("name")
        or ""
    )
    if creator:
...
def _new_comments_in_window(
    comments: list[dict[str, Any]], start: str | None, end: str | None
) -> list[dict[str, Any]]:
    return [
        comment
...
def _gentoo_bug_metadata(bug: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        if isinstance(bug.get("creator_detail"), dict)
        else {},
...
def _gentoo_content(
    ...
    if new_comments:
        ...
        for comment in new_comments[:10]:
            ...
            lines.append(
                f"- Comment #{comment.get('count')} by {creator} at {created}: {text}"
            )
    return "\n".join(lines)
...
def parse_html_entries(spec: SourceSpec, text: str) -> list[SourceEntry]:
    ...
    relevant_links = [
        ...
        if _looks_security_related(link[1]) or _looks_security_related(link[0])
    ]
    entries: list[SourceEntry] = []
...
def filter_entries_by_window(
    ...
    return [
        ...
        if in_window(
            _entry_window_timestamp(entry, prefer_published=prefer_published),
            start,
            end,
            include_undated=include_undated,
        )
...
def _entry_window_timestamp(
    entry: SourceEntry, prefer_published: bool = True
) -> str | None:
    if prefer_published:
...
def _xml_text(
    node: ET.Element, path: str, namespaces: dict[str, str] | None = None
) -> str:
...
def _looks_security_related(text: str) -> bool:
    ...
    return any(
        marker in lowered
        for marker in ["cve-", "security", "vulnerab", "glsa", "announce", "advisory"]
    )
