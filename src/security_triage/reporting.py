...
def write_document(path: str | Path, document: dict[str, Any], output_format: str = "json") -> None:
...
def render_discovery_markdown(document: dict[str, Any]) -> str:
    ...
    for record in document.get("records", []):
        ...
        if sbom_assessment.get("reason"):
            lines.append(f"- SBOM match assessment: {sbom_assessment.get('status')} - {sbom_assessment.get('reason')}")
        if record.get("proposed_issue"):
            proposed = record["proposed_issue"]
            lines.extend([f"- Proposed issue: {proposed.get('title')}", f"- Labels: {', '.join(proposed.get('labels') or [])}"])
        if record.get("proposed_update"):
            update = record["proposed_update"]
            lines.append(f"- Proposed update: #{update.get('issue')} {update.get('title')}")
            ...
            if changes:
                ...
                lines.extend(f"- {_format_detected_change(change)}" for change in changes)
            ...
            if update.get("comment_body"):
                lines.extend(["", "Comment preview:", "", "```markdown", update["comment_body"], "```"])
            if update.get("body_update_mode") == "additive_guarded":
                lines.extend(["", "Proposed additive issue body update: existing issue content is preserved by guardrails."])
            elif update.get("updated_body_diff"):
                lines.extend(["", "Issue body diff:", "", "```diff", update["updated_body_diff"], "```"])
        if record.get("manual_review_reasons"):
            lines.append(f"- Manual review: {'; '.join(record.get('manual_review_reasons') or [])}")
        lines.append("")
...
def _discovery_record_title(record: dict[str, Any], extraction: dict[str, Any]) -> str:
    ...
    package_name = extraction.get("package_name") or record.get("raw_advisory_id") or "unknown"
...
def _format_detected_change(change: dict[str, Any]) -> str:
    ...
    if kind == "bugzilla_severity":
        source = f"; source: {change.get('source_url')}" if change.get("source_url") else ""
...
def render_cleanup_markdown(document: dict[str, Any]) -> str:
    ...
    for record in document.get("records", []):
        ...
        if matches:
            match_text = "; ".join(f"{match.get('name')} {match.get('versionInfo')}" for match in matches)
