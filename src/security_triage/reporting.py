...
def render_review_plan_markdown(advisory_repo: str, review_repo: str) -> str:
    lines = [
        "# Flatcar Security Triage Review Plan",
        "",
        f"Advisory repo: `{advisory_repo}`",
        f"Review repo: `{review_repo}`",
        "",
        "## Safety Model",
        "",
        "- `discovery` and `cleanup` are read-only and write local reports.",
        "- `review render` previews the exact review issue body locally.",
        "- `review create` opens or reuses human review issue(s).",
        "- `review apply` mutates GitHub only after a completed review issue is closed.",
        "- Mutation flags are explicit; unchecked, conflicting, or stale actions are skipped.",
        "",
        "## Local Dry Run",
        "",
        "```bash",
        ...
        "  --issue-number REVIEW_ISSUE_NUMBER \\",
        f"  --advisory-repo {advisory_repo} \\",
        f"  --review-repo {review_repo} \\",
