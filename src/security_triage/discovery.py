...
from .issue_updates import append_field_values, removal_guard_violations, set_field_if_placeholder
...
class DiscoveryWorkflow:
    ...
    def run(self, entries: list[SourceEntry], window_start: str, window_end: str) -> dict[str, Any]:
        records: list[dict[str, Any]] = []
        errors: list[dict[str, Any]] = []
        total = len(entries)
        self.progress_logger.info(f"Processing {total} upstream source entr{'y' if total == 1 else 'ies'}")
        for index, entry in enumerate(entries, start=1):
            record_id = _record_id(entry)
            self.progress_logger.info(f"[{index}/{total}] Processing {entry.source} entry {entry.entry_id}: {entry.title[:100]}")
            self.debug_logger.log("discovery_source_entry", record_id=record_id, source=entry.source, raw_entry=entry.raw, content=entry.content)
            try:
                record = self._process_entry(record_id, entry)
            except Exception as exc:
                self.progress_logger.info(f"[{index}/{total}] Entry {entry.entry_id} failed; recording manual review: {exc}")
                errors.append({"record_id": record_id, "source": entry.source, "error": str(exc)})
                record = self._manual_record(record_id, entry, str(exc))
            records.append(record)
            decision = record.get("decision", {})
            package_name = record.get("llm_extraction", {}).get("package_name") or "unknown"
            self.progress_logger.info(
                f"[{index}/{total}] Finished {package_name}: {decision.get('action')} ({decision.get('confidence')})"
            )
            self.debug_logger.log("discovery_final_record", record_id=record_id, record=record)
        ...
        document = {
            ...
            "processing_window": {"start": window_start, "end": window_end, "timezone": "UTC"},
    ...
    def _process_entry(self, record_id: str, entry: SourceEntry) -> dict[str, Any]:
        ...
        self.progress_logger.info(f"Extracted package {extraction.get('package_name') or 'unknown'} with {len(extraction.get('cves') or [])} CVE(s)")
        ...
        self.progress_logger.info(f"Found {len(existing_issue_matches)} existing issue match(es)")
        covered_match = _already_covered_by_existing_issue(extraction, existing_issue_matches)
        upstream_activity = _upstream_activity(entry, extraction, existing_issue_matches)
        if covered_match and not upstream_activity["requires_issue_update"]:
            ...
            return _already_tracked_record(record_id, entry, extraction, sbom_matches, existing_issue_matches, covered_match, upstream_activity)
        evidence_bundle = build_discovery_evidence_bundle(record_id, entry, extraction, sbom_matches, existing_issue_matches)
        self.debug_logger.log("discovery_evidence_bundle", record_id=record_id, bundle=evidence_bundle)
        ...
        if upstream_activity["requires_issue_update"] and existing_issue_matches and decision["action"] == "ignore":
        ...
        if decision["action"] == "create_issue":
            ...
            proposed_update = _proposed_update(entry, extraction, existing_issue_matches, upstream_activity)
    ...
    def _manual_record(self, record_id: str, entry: SourceEntry, reason: str) -> dict[str, Any]:
        return {
            "record_id": record_id,
            "source": entry.source,
            "source_url": entry.source_url,
            ...
            **_source_detail_fields(entry, _empty_upstream_activity()),
            ...
            "existing_issue_matches": [],
            "decision": {"action": "needs_manual_review", "confidence": "low", "reason": reason},
            "proposed_issue": None,
            "proposed_update": None,
...
def _record_id(entry: SourceEntry) -> str:
    digest = hashlib.sha256(f"{entry.source}:{entry.entry_id}:{entry.source_url}".encode("utf-8")).hexdigest()[:16]
...
def _sources_summary(entries: list[SourceEntry]) -> list[dict[str, Any]]:
    ...
    for entry in entries:
        source = summary.setdefault(entry.source, {"source": entry.source, "urls": [], "entry_count": 0})
...
def _proposed_issue(extraction: dict[str, Any], relevance: dict[str, Any]) -> dict[str, Any]:
    package_name = extraction["package_name"]
    labels = issue_labels(extraction.get("cvss_scores"), relevance.get("scope"), extraction.get("scope_assessment"))
...
def _proposed_update(
    ...
    if upstream_activity.get("new_aliases"):
        additions.append(f"Add Gentoo aliases/CVEs: {', '.join(upstream_activity['new_aliases'])}")
    action_needed = extraction.get("action_needed")
    if action_needed and action_needed != "TBD" and _value_missing_from_issue(action_needed, matches[0]):
...
def _already_covered_by_existing_issue(extraction: dict[str, Any], matches: list[dict[str, Any]]) -> dict[str, Any] | None:
    extracted_ids = {str(item).strip().upper() for item in extraction.get("cves") or [] if str(item).strip()}
    if not extracted_ids:
        return None
    for match in matches:
        issue_ids = {str(item).strip().upper() for item in match.get("cves") or [] if str(item).strip()}
...
def _already_tracked_record(
    ...
    relevance = {
        ...
        "reasons": ["Existing issue already covers the extracted CVE or upstream advisory IDs."],
        "evidence": [f"Existing issue #{covered_match.get('issue')}: {covered_match.get('title')}"],
    }
    decision = {
        "action": "ignore",
        "confidence": "high",
...
def _source_detail_fields(entry: SourceEntry, upstream_activity: dict[str, Any]) -> dict[str, Any]:
    return {
...
def _upstream_activity(entry: SourceEntry, extraction: dict[str, Any], matches: list[dict[str, Any]]) -> dict[str, Any]:
    if not matches:
    ...
    aliases = [str(alias) for alias in entry.metadata.get("alias", []) if str(alias).strip()]
    alias_cves = extract_cves(" ".join(aliases))
    new_aliases = [cve for cve in alias_cves if cve not in existing_cves]
    new_references = [reference for reference in _activity_references(entry) if reference and reference not in existing_body]
    ...
    if new_references:
        recommended_additions.append(f"Review upstream references: {', '.join(new_references[:5])}")
    if entry.description and _compact(entry.description) not in _compact(existing_body):
        recommended_additions.append(f"Review Bugzilla description: {_truncate(neutralize_mentions(entry.description), 300)}")
    ...
    requires_issue_update = bool(new_aliases or extracted_new_cves or recommended_additions)
    return {
...
def _new_cves_for_issue(cves: list[Any], match: dict[str, Any]) -> list[str]:
    ...
    for cve in cves:
        ...
        if normalized and normalized != "TBD" and normalized not in issue_cves and normalized not in new_cves:
...
def _detected_changes(upstream_activity: dict[str, Any], new_cves: list[str]) -> list[dict[str, Any]]:
    changes: list[dict[str, Any]] = []
    if new_cves:
        changes.append({"kind": "new_extracted_cves", "values": new_cves, "reason": "Extracted CVEs are missing from the matched issue."})
    if upstream_activity.get("new_aliases"):
        changes.append({"kind": "new_bugzilla_aliases", "values": upstream_activity["new_aliases"], "reason": "Bugzilla aliases include CVEs missing from the matched issue."})
    if upstream_activity.get("new_references"):
        changes.append({"kind": "new_references", "values": upstream_activity["new_references"], "reason": "Bugzilla URL, see_also, or source references are missing from the matched issue."})
...
def _activity_references(entry: SourceEntry) -> list[str]:
    references: list[str] = []
    for reference in [entry.metadata.get("url"), *entry.metadata.get("see_also", []), *entry.references]:
        if reference and reference != entry.source_url and str(reference) not in references:
...
def _updated_issue_body(
    ...
    existing_body = str(match.get("body") or "")
    package_name = parsed.get("name") or extraction.get("package_name") or match.get("package") or ""
    if not package_name:
        return None
    if not existing_body.strip():
        cves = _dedupe_strings([*(parsed.get("cves") or []), *(extraction.get("cves") or []), *(upstream_activity.get("new_aliases") or [])], upper=True)
        cvss_scores = _dedupe_strings([*(parsed.get("cvss_scores") or []), *(extraction.get("cvss_scores") or [])])
    ...
    updated = append_field_values(updated, "CVEs", [*(extraction.get("cves") or []), *(upstream_activity.get("new_aliases") or [])])
    updated = append_field_values(updated, "CVSSs", extraction.get("cvss_scores") or [])
    updated = append_field_values(updated, "refmap.gentoo", _gentoo_refs_for_update(entry, extraction))
    updated = set_field_if_placeholder(updated, "Action Needed", extraction.get("action_needed"))
...
def _gentoo_refs_for_update(entry: SourceEntry, extraction: dict[str, Any]) -> list[str]:
    candidates = [extraction.get("gentoo_ref"), entry.source_url, entry.metadata.get("url")]
    candidates.extend(entry.metadata.get("see_also", []) or [])
    candidates.extend(entry.references)
    return _dedupe_strings([candidate for candidate in candidates if _is_gentoo_ref(candidate)])
...
def _upstream_update_comment_body(entry: SourceEntry, upstream_activity: dict[str, Any], additions: list[str]) -> str | None:
    if not additions and not upstream_activity.get("new_comments"):
        return None
    lines = ["Gentoo Bugzilla has new or changed upstream context for this advisory.", ""]
    if additions:
    ...
    if upstream_activity.get("new_comments"):
        ...
        lines.append("")
    lines.extend([f"Source: {entry.source_url}", "", "This is a guarded automation recommendation; maintainers should review before editing the advisory body."])
    return "\n".join(lines)
...
def _record_evidence(relevance: dict[str, Any], sbom_matches: list[dict[str, Any]], issue_matches: list[dict[str, Any]]) -> list[str]:
    evidence = list(relevance.get("evidence") or [])
    for match in sbom_matches:
        evidence.append(f"SBOM package match: {match.get('name')} {match.get('versionInfo')}")
    for match in issue_matches:
        evidence.append(f"Existing issue match: #{match.get('issue')} {match.get('title')}")
    return evidence
