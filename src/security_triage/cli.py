...
from .reporting import render_cleanup_markdown, render_discovery_markdown, write_document, write_markdown
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version=f"security-triage schema {SCHEMA_VERSION}")
    subparsers = parser.add_subparsers(dest="command", required=True)

    discovery = subparsers.add_parser("discovery", help="Run new vulnerability discovery")
    _add_common_io_args(discovery, "reports/discovery.json")
    _add_model_args(discovery)
    discovery.add_argument("--source-fixture", help="JSON/YAML fixture containing upstream source entries")
    discovery.add_argument("--issues-fixture", help="JSON/YAML fixture containing GitHub issues")
    discovery.add_argument("--sbom-fixture", help="SPDX JSON fixture for the Stable production SBOM")
    discovery.add_argument("--window-start", help="Processing window start ISO timestamp")
    discovery.add_argument("--window-end", help="Processing window end ISO timestamp")
    discovery.add_argument("--window-days", type=int, default=7, help="Default processing window length when explicit bounds are omitted")
    discovery.add_argument("--include-undated-source-entries", action="store_true", help="Include live source entries that do not expose published or updated timestamps")
    discovery.add_argument("--include-optional-sources", action="store_true", help="Include optional Red Hat vulnerability source")
    discovery.add_argument("--oss-security-cache-dir", help="Directory to cache fetched oss-security message HTML between runs")
    discovery.add_argument("--go-vulndb-cache-dir", help="Directory to cache fetched Go vulnerability database advisory JSON between runs")
    discovery.add_argument("--rustsec-cache-dir", help="Directory to cache fetched RustSec advisory Markdown between runs")
    discovery.add_argument("--apply-actions", action="store_true", help="Apply guarded GitHub actions after producing validated records")
    discovery.add_argument("--enable-create-issues", action="store_true", help="Allow creating new GitHub issues when --apply-actions is set")
    discovery.add_argument("--enable-update-issues", action="store_true", help="Allow commenting on existing issues with recommended updates when --apply-actions is set")

    cleanup = subparsers.add_parser("cleanup", help="Run advisory cleanup recommendation")
    _add_common_io_args(cleanup, "reports/cleanup.json")
    _add_model_args(cleanup)
    cleanup.add_argument("--issues-fixture", help="JSON/YAML fixture containing GitHub issues")
    cleanup.add_argument("--sbom-fixture", help="SPDX JSON fixture for the Stable production SBOM")
    cleanup.add_argument("--apply-actions", action="store_true", help="Apply guarded GitHub actions after producing validated records")
    cleanup.add_argument("--enable-post-cleanup-comments", action="store_true", help="Allow posting cleanup comments when --apply-actions is set")
    cleanup.add_argument("--enable-close-issues", action="store_true", help="Allow closing remediated issues when --apply-actions is set")
    return parser
...
def run_discovery_command(args: argparse.Namespace) -> int:
    ...
    progress.info(f"Loaded {len(entries)} upstream source entr{'y' if len(entries) == 1 else 'ies'}")
    workflow = DiscoveryWorkflow(model_client, sbom_index, issues, debug_logger, progress)
    ...
    if args.apply_actions:
        ...
        flags = ActionFlags(create_issues=args.enable_create_issues, update_existing_issues=args.enable_update_issues)
        ...
        progress.info(f"Completed {len(action_results)} guarded GitHub action result(s)")
...
def run_cleanup_command(args: argparse.Namespace) -> int:
    ...
    if args.apply_actions:
        ...
        flags = ActionFlags(post_cleanup_comments=args.enable_post_cleanup_comments, close_issues=args.enable_close_issues)
        runner = GitHubActionRunner(GitHubIssueClient(), flags, debug_logger)
...
def _add_common_io_args(parser: argparse.ArgumentParser, default_output: str) -> None:
    parser.add_argument("--output", default=default_output, help="Machine-readable JSON/YAML output path")
    parser.add_argument("--format", choices=["json", "yaml"], help="Output format; inferred from --output by default")
    parser.add_argument("--markdown-output", help="Optional human-readable Markdown report path")
...
def _add_model_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--model", choices=["heuristic", "github", "foundry", "fixture"], default="heuristic", help="Model provider to use")
    parser.add_argument("--model-fixture", help="JSON/YAML fixture with model responses; implies --model fixture")
    parser.add_argument("--extraction-model", help="Optional cheap-tier GitHub Models model name used only for the first-pass extract_advisory call (e.g. openai/gpt-4o-mini). Requires --model github.")
    parser.add_argument("--foundry-endpoint", help="Microsoft Foundry/Azure OpenAI endpoint base URL (env: FOUNDRY_ENDPOINT; required for --model foundry)")
    parser.add_argument("--foundry-deployment", help=f"Foundry deployment for reasoning and cleanup (env: FOUNDRY_DEPLOYMENT; default: {DEFAULT_FOUNDRY_DEPLOYMENT})")
    parser.add_argument("--foundry-extraction-deployment", help=f"Foundry deployment for extract_advisory calls (env: FOUNDRY_EXTRACTION_DEPLOYMENT; default: {DEFAULT_FOUNDRY_EXTRACTION_DEPLOYMENT})")
    parser.add_argument("--foundry-api-version", help=f"Foundry chat completions API version: dated value for legacy deployment endpoints, v1/preview for OpenAI v1 endpoints (env: FOUNDRY_API_VERSION; default: {DEFAULT_FOUNDRY_API_VERSION})")
    parser.add_argument("--prompt-cache-dir", help="Directory to cache model prompt responses between runs (env: FLATCAR_PROMPT_CACHE_DIR)")


def _model_client(args: argparse.Namespace, debug_logger: DebugLogger, progress: ProgressLogger) -> BaseModelClient:
    ...
    if args.model == "github":
        ...
        primary = GitHubModelsClient(debug_logger=debug_logger, progress_logger=progress, prompt_cache=cache)
        extraction_model = getattr(args, "extraction_model", None)
        if extraction_model:
            progress.info(f"Using cheap-tier extraction model for first pass: {extraction_model}")
    ...
    if args.model == "foundry":
        if getattr(args, "extraction_model", None):
            raise ValueError("--extraction-model requires --model github; use --foundry-extraction-deployment with --model foundry")
        ...
        progress.info(f"Using Foundry extraction deployment for first pass: {extraction_deployment}")
...
def _prompt_cache(args: argparse.Namespace, progress: ProgressLogger) -> PromptCache | None:
    directory = getattr(args, "prompt_cache_dir", None) or os.getenv("FLATCAR_PROMPT_CACHE_DIR")
    if not directory:
        return None
